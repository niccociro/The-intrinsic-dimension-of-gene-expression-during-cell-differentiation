...
def download_data(data_file_folder = '', verbose = True):
    ...
    adata_raw=scanpy.read_h5ad(data_file_folder + 'Pancreas_raw_counts_and_metadata.h5ad')

    mtx_rawcounts = adata_raw.X
    df_meta = adata_raw.obs
    df_meta = df_meta.rename(columns={'day': 'stage'})
    df_meta = df_meta.rename(columns={'clusters_fig2_final': 'celltype'})
