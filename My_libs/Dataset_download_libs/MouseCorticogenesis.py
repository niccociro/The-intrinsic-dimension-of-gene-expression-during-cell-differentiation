...
def download_data(data_file_folder = '', verbose = True):
    ...
    adata_raw = anndata.read_h5ad(data_file_folder + 'mouse_cortex_raw_meta.h5ad')
    
    df = adata_raw.obs
    df.rename(columns={'Gral_cellType': 'celltype'}, inplace=True)
    df.rename(columns={'orig_ident': 'stage'}, inplace=True)
